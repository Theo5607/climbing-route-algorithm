...
let affiche_mur p =
	...
	Array.iter (fun (x,y) -> draw_circle (x*dx) ((18-y)*dy) 10. Color.blue) p

let affiche_pos p pos_tab =
	let xm,ym = centre p pos_tab in
	let m = Vector2.create (foi (xm*dx)) (foi ((18 - ym)*dy)) in

	draw_line_ex m (Vector2.create (foi ((fst p.(pos_tab.(0)))*dx)) (foi ((18-(snd p.(pos_tab.(0))))*dy))) 4. Color.red;
